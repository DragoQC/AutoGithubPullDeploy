...
remove_dotnet_home() {
  ...
  if [[ -d "$dotnet_dir" ]]; then
    rm -rf "$dotnet_dir"
    echo "$(c_dotnet "Removed .NET SDK directory: $dotnet_dir")"
  fi
}
...
cleanup_database_assets() {
  ...
  if [[ ! -f "$DB_CREDENTIALS_FILE" ]]; then
    echo "$(c_db "No DB credentials file found at $DB_CREDENTIALS_FILE")"
    return 0
  fi
  ...
  if [[ -z "$db_name" || -z "$db_user" ]]; then
    echo "$(c_db "DB credentials file is missing DB_NAME/DB_USER; skipping DB cleanup.")"
    return 0
  fi
  ...
  if command -v mariadb >/dev/null 2>&1; then
    ...
    echo "$(c_db "No MariaDB client found; skipping DB cleanup.")"
    return 0
  fi
  ...
  echo "$(c_db "Attempting DB cleanup for database '$db_name' and user '$db_user'@'$db_host'...")"
  ...
  if sudo_if_needed "$client" -e "DROP DATABASE IF EXISTS \`$esc_db\`; DROP USER IF EXISTS '$esc_user'@'$esc_host'; FLUSH PRIVILEGES;" >/dev/null 2>&1; then
    root_drop_ok=1
    echo "$(c_db "Dropped database and user via privileged DB connection.")"
  fi
  ...
  if [[ $root_drop_ok -eq 0 ]]; then
    if [[ -n "$db_pass" ]] && MYSQL_PWD="$db_pass" "$client" -u"$db_user" -e "DROP DATABASE IF EXISTS \`$esc_db\`;" >/dev/null 2>&1; then
      user_drop_ok=1
      echo "$(c_db "Dropped database using app DB credentials.")"
    fi

    if [[ -n "$db_pass" ]] && MYSQL_PWD="$db_pass" "$client" -u"$db_user" -e "DROP USER IF EXISTS '$esc_user'@'$esc_host';" >/dev/null 2>&1; then
      user_drop_user_ok=1
      echo "$(c_db "Dropped DB user using app DB credentials.")"
    fi
  fi

  if [[ $root_drop_ok -eq 0 && $user_drop_ok -eq 0 ]]; then
    echo "$(c_db "Could not drop database automatically. Check DB root access and credentials.")"
    return 0
  fi
  ...
  if [[ "$remove_creds" =~ ^[Yy]$ ]]; then
    ...
    echo "$(c_db "Removed $DB_CREDENTIALS_FILE")"
  fi
}
...
remove_toolchains() {
  ...
  if [[ "$node_marker" == "1" || -x "$(command -v node 2>/dev/null || true)" || -x "$(command -v npm 2>/dev/null || true)" ]]; then
    echo "$(c_node "Removing Node.js/npm...")"
    case "$os" in
      debian)
  ...
  if [[ "$dotnet_marker" == "1" || -d "$HOME/.dotnet" || -d "/root/.dotnet" ]]; then
    echo "$(c_dotnet "Removing user-local .NET SDK directories...")"
  ...
  echo "$(c_menu "Toolchain cleanup complete.")"
}
